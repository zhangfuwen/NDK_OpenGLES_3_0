...
int ObjLoader::LoadMtlLib(std::string path) {
    ...
    int ret = handycpp::file::for_each_line(path, [&](int lineNo, std::string line) {
        if (handycpp::string::starts_with(line, "newmtl")) {
            ...
            line = line.substr(1);
            PARSE(Ns) else PARSE(Ni) else PARSE(d) else PARSE(Tr) else PARSE(Tf) else PARSE(
                    illum) else PARSE(Ka) else PARSE(Kd) else PARSE(Ks) else PARSE(Ke) else if (
                    handycpp::string::starts_with(line, "map_") ||
